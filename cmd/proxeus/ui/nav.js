(function(){
  ...
  var ICON_URL = PREFIX + '/proxeus/static/proxeus-icon.svg';
  var BACKENDS_PATH = PREFIX + '/backends';
  var API_URL = PREFIX + '/proxeus/api/backends.json';
  ...
  function fixBackendsHref(a) {
    ...
    if (pathname && pathname.indexOf(prefix) === 0) {
      ...
      if (!a.dataset.proxeusFixed) {
        a.dataset.proxeusFixed = '1';
  ...
  function customiseHeader() {
    ...
    while ((node = walker.nextNode())) {
      if (node.nodeValue.indexOf('Prometheus') !== -1) {
        node.nodeValue = node.nodeValue.replace(/Prometheus/g, 'Proxeus');
      }
    }
  ...
  function customiseDropdown() {
    ...
    menus.forEach(function(menu) {
      if (menu.dataset.proxeusProcessed) return;
      ...
      if (!menu.querySelector('[data-proxeus-backends]') && serverStatusLabel) {
        ...
        if (templateItem) {
          ...
          backendsLink.setAttribute('data-proxeus-backends', '');
      ...
      menu.dataset.proxeusProcessed = '1';
    });
  }

  // --- Backends page renderer ---
  var isBackendsPage = !!window.__proxeusBackends;
  ...
  function renderBackendsPage() {
    ...
    var container = document.getElementById('proxeus-backends-root');
    if (!container) {
      container = document.createElement('div');
      container.id = 'proxeus-backends-root';
  ...
  function loadBackendsData(container) {
    var statusEl = document.getElementById('proxeus-backends-status');
  ...
  function renderBackendsData(container, data) {
    ...
    var html = '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;">' +
      ...
      '<span id="proxeus-backends-status" style="font-size:0.75rem;color:#868e96;">Updated ' + now + '</span>' +
      '</div>';
